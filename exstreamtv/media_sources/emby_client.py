"""
Emby Media Source Integration

Connects to Emby Media Server to scan and import media libraries.
Emby's API is very similar to Jellyfin (as Jellyfin was forked from Emby).
"""
...
class EmbyMediaSource(MediaSource):
    ...
    Connects to an Emby server using an API key to:
    - List available libraries
    - Scan and import media items
    - Get streaming URLs for playback
    
    Authentication:
        Uses Emby API keys. Get your API key from:
    ...
    def headers(self) -> dict[str, str]:
        """Get authentication headers for Emby API."""
        return {
            ...
            "X-Emby-Device-Name": "EXStreamTV",
            "X-Emby-Device-Id": "exstreamtv",
        }
    ...
    async def _request(
        ...
        try:
            ...
            self.status = MediaSourceStatus.ERROR
            logger.exception(f"Emby API error: {e}")
            return None
    
    async def connect(self) -> bool:
        """Connect to the Emby server and verify authentication."""
        ...
        self._server_name = result.get("ServerName", "Emby Server")
        ...
        logger.info(f"Connected to Emby: {self._server_name} v{self._server_version}")
        return True
    ...
    async def disconnect(self) -> None:
        ...
        logger.info(f"Disconnected from Emby: {self.name}")
    ...
    async def get_libraries(self) -> list[MediaLibrary]:
        """Get available libraries from Emby."""
    ...
    async def scan_library(self, library_id: str) -> list[MediaSourceItem]:
        """Scan an Emby library and return all items."""
        ...
        result = await self._request(
            ...
            params={
                ...
                "Fields": "Path,Overview,Genres,Studios,People,MediaStreams,ProviderIds,DateCreated",
                "StartIndex": 0,
                "Limit": 10000,
            },
        )
        ...
        self.status = MediaSourceStatus.CONNECTED
        logger.info(f"Scanned Emby library: {len(items)} items")
        return items
    ...
    def _parse_item(
        ...
        """Parse Emby item into MediaSourceItem."""
        try:
            emby_type = item_data.get("Type", "")
            ...
            # Get duration in ms (Emby uses ticks like Jellyfin)
            ...
            logger.warning(f"Error parsing Emby item: {e}")
            return None
    ...
    async def refresh_library(self, library_id: str) -> bool:
        """Trigger an Emby library refresh."""
        result = await self._request(
