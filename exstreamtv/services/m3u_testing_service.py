...
class M3UTestingService:
    """Scheduled testing of M3U streams (uses short-lived sync DB sessions per run)."""
    ...
    async def test_all_streams(self) -> None:
        ...
        try:
            streams = list(
                db.scalars(
                    select(M3UStreamSource).where(M3UStreamSource.is_active.is_(True))
                ).all()
            )
    ...
    async def test_stream_source(self, stream_id: int) -> dict[str, Any] | None:
        """Test a single stream source"""
        db = get_sync_session()
        try:
            stream = db.scalars(
                select(M3UStreamSource).where(M3UStreamSource.id == stream_id)
            ).first()
    ...
    def close(self) -> None:
        """Compatibility no-op (sessions are per-call)."""
        return None
