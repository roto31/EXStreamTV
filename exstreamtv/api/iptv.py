    ...
    try:
        ...
        for channel in channels:
            ...
            for schedule_item in schedule_items:
                ...
                # Skip placeholder titles generated by importers (e.g. "Item 332245").
                # Both playout_item.title, playout_item.custom_title, and media_item.title
                # can contain these placeholders from ErsatzTV/StreamTV import.
                if title and re.match(r"^Item \d+$", title):
                    # Try Plex metadata fields for a real title
                    real_title = None
                    if media_item:
                        real_title = (
                            getattr(media_item, 'original_title', None)
                            or getattr(media_item, 'sort_title', None)
                            or getattr(media_item, 'ai_enhanced_title', None)
                            or getattr(media_item, 'episode_title', None)
                        )
                        # Try series + episode format
                        if not real_title:
                            series = getattr(media_item, 'series_title', None) or getattr(media_item, 'show_title', None)
                            ep_title = getattr(media_item, 'episode_title', None)
                            if series:
                                real_title = f"{series} - {ep_title}" if ep_title else series
